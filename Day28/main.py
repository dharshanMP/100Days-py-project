from tkinter import *
import math

PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None


#---------------------------------TIMER RESET----------------------------------#
def reset_timer():
    window.after_cancel(timer)
    canvas.itemconfig(timer_text, text ="00:00")
    timer_label.config(text="Timer")
    end.config(text="")
    global reps
    reps = 0



#------------------------------TIMER MECHANISM------------------------------#

def start_time():
    global reps
    reps += 1


    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break_sec = LONG_BREAK_MIN * 60

    count_down(work_sec)
    if reps % 8==0:
        count_down(long_break_sec)
        timer_label.config(text="break", fg=RED)
    elif reps %2 == 0:
        count_down(short_break_sec)
        timer_label.config(text="break", fg=PINK)
    else:
        count_down(work_sec)    
        end = Label(fg=GREEN, bg=YELLOW)


#--------------------------------COUNTDOWN MECHANISM--------------------#
def count_down(count):

    count_min = math.floor(count/60)
    count_sec = count % 60

    if count_sec < 10:
        count_sec =f"0{count_sec}"

    canvas.itemconfig(timer_text, text = f"{count_min}:{count_sec}")
    if count > 0:
       global timer
       timer = window.after(1000, count_down, count -1)
    else:
        start_time()    
        mark = ""
        work_session = (math.floor(reps/2))
        for _ in range(work_session):
            mark += "✔️"
        end.config(text=mark)    






#------------------------------------UI SETUP-------------------------#

window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW )



#Timer
timer_label = Label(text="Timer", font=(FONT_NAME, 35, "bold"),bg=YELLOW,fg=GREEN)
timer_label.grid(row=1, column=2)


my_image = PhotoImage(file="tomato.png")
canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=False)
canvas.create_image(100,112,image=my_image)



#text
timer_text = canvas.create_text(100, 130, text="00:00", fill="green",font=(FONT_NAME, 35, "bold") )
canvas.grid(row = 2, column= 2)


#start button

start_button = Button(text="Start",highlightthickness=False, command=start_time)
start_button.config(padx=10, pady=5)
start_button.grid(row=3, column=1)


#reset button

reset_button = Button(text="Reset", highlightthickness=False, command=reset_timer)
reset_button.config(padx=10, pady=5)
reset_button.grid(row=3,column=3)

#end label

end = Label(fg=GREEN, bg=YELLOW)
end.grid(row=4, column=3)





window.mainloop()